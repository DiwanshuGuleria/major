...
public class EdaStego {
	...
	private BufferedImage add_text(BufferedImage image, String text) {
		...
		try {
			encode_text(img, len, 0); // 0 first positing
	...
	private byte[] encode_text(byte[] image, byte[] addition, int offset) {
		...
		for (int i = 0; i < addition.length; ++i) {
			Byte x1,x2;
			int add = addition[i];
			for (int bit = 7; bit >= 0; bit-=2, offset+=2) 
			{
				b= (add >>> bit) & 1;
				c = (add >>> bit-1) & 1;
	...
	private byte[] decode_text(byte[] image) {
		...
		for (int b = 0; b < result.length; b+=1) {
			for (int i = 0; i < 8; i+=2, offset+=2) {
				x1=image[offset];
				x2=image[offset+1];
				o=x1.intValue()%2;
				p=((x2.intValue()+x1.intValue()/2))%2;

				result[b] = (byte) ((result[b] << 1) | o);
				result[b] = (byte) ((result[b] << 1) | p);
			}
		}
		return result;
	}
